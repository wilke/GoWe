...
var templates = map[string]string{
	// Shared pagination component — called with {{template "pagination" .Pagination}}
	"components/pagination": `{{define "pagination"}}
{{if or .HasPrev .HasMore (gt .Total 0)}}
<div class="flex flex-col sm:flex-row items-center justify-between gap-4">
    <!-- Page size -->
    <div class="flex items-center space-x-2 text-sm text-gray-500">
        <span>Show</span>
        {{$fp := .FilterParams}}{{$curLimit := .Limit}}
        {{range .PageSizes}}
        <a href="?limit={{.}}{{$fp}}"
           class="px-2 py-1 rounded {{if eq . $curLimit}}bg-indigo-100 text-indigo-800 font-medium{{else}}hover:bg-gray-100{{end}}">
            {{.}}
        </a>
        {{end}}
        <span>per page</span>
    ...
    <div class="flex items-center space-x-1">
        {{if .HasPrev}}
        <a href="?offset=0&limit={{.Limit}}{{.FilterParams}}"
        ...
        <a href="?offset={{.PrevOffset}}&limit={{.Limit}}{{.FilterParams}}"
        ...
        {{if .HasMore}}
        <a href="?offset={{.NextOffset}}&limit={{.Limit}}{{.FilterParams}}"
        ...
        <a href="?offset={{.LastOffset}}&limit={{.Limit}}{{.FilterParams}}"
    ...
    <span class="text-sm text-gray-500">
        {{add .Offset 1}}-{{add .Offset .ItemCount}} of {{.Total}}
    </span>
</div>
{{end}}
{{end}}`,
...
<div class="px-4 py-6 sm:px-0">
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{.SearchQuery}}" placeholder="Name or ID..."
            ...
            {{with .ClassFilter}}<input type="hidden" name="class" value="{{.}}">{{end}}
            <button type="submit"
    ...
    <!-- Top Pagination -->
    <div class="mb-4">{{template "pagination" .Pagination}}</div>
    ...
    <!-- Bottom Pagination -->
    <div class="mt-4">{{template "pagination" .Pagination}}</div>
</div>
{{end}}`,
...
<div class="px-4 py-6 sm:px-0">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center space-x-2">
            ...
            <div class="inline-flex rounded-md shadow-sm">
                <a href="?view=cards{{with .StateFilter}}&amp;state={{.}}{{end}}{{with .SearchQuery}}&amp;search={{urlquery .}}{{end}}{{with .DateStart}}&amp;date_start={{.}}{{end}}{{with .DateEnd}}&amp;date_end={{.}}{{end}}"
                ...
                <a href="?view=table{{with .StateFilter}}&amp;state={{.}}{{end}}{{with .SearchQuery}}&amp;search={{urlquery .}}{{end}}{{with .DateStart}}&amp;date_start={{.}}{{end}}{{with .DateEnd}}&amp;date_end={{.}}{{end}}"
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-medium text-gray-500 mb-1">Search</label>
                <input type="text" name="search" value="{{.SearchQuery}}" placeholder="Workflow name or submission ID..."
                       class="block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 px-3 py-1.5 border">
            </div>
            <!-- State Filter -->
            <div>
            ...
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="?sort=workflow_name&amp;dir={{if and (eq .SortBy "workflow_name") (eq .SortDir "asc")}}desc{{else}}asc{{end}}&amp;limit={{.Pagination.Limit}}{{.FilterBase}}"
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="?sort=created_at&amp;dir={{if and (eq .SortBy "created_at") (eq .SortDir "desc")}}asc{{else}}desc{{end}}&amp;limit={{.Pagination.Limit}}{{.FilterBase}}"
                           class="flex items-center hover:text-gray-700">
                            Created
                            {{if eq .SortBy "created_at"}}<span class="ml-1">{{if eq .SortDir "asc"}}&#9650;{{else}}&#9660;{{end}}</span>{{end}}
                        </a>
                    </th>
                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="?sort=state&amp;dir={{if and (eq .SortBy "state") (eq .SortDir "asc")}}desc{{else}}asc{{end}}&amp;limit={{.Pagination.Limit}}{{.FilterBase}}"
                           class="flex items-center justify-end hover:text-gray-700">
                            Status
                            {{if eq .SortBy "state"}}<span class="ml-1">{{if eq .SortDir "asc"}}&#9650;{{else}}&#9660;{{end}}</span>{{end}}
                        </a>
                    </th>
                </tr>
            </thead>
                ...
                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='/submissions/{{.ID}}'">
                    ...
                    <td class="px-4 py-3">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span class="flex-shrink-0 text-xs text-gray-500">{{.TaskSummary.Success}}/{{.TaskSummary.Total}} tasks</span>
                        </div>
                        {{else}}
                        <span class="text-xs text-gray-500">{{if .QueuePosition}}Queue #{{.QueuePosition}}{{else}}No tasks{{end}}</span>
                        {{end}}
                    </td>
                    <!-- Created -->
    ...
    <!-- Bottom Pagination -->
    <div class="mt-6">{{template "pagination" .Pagination}}</div>
</div>
{{end}}`,
...
<div class="px-4 py-6 sm:px-0">
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <form method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-medium text-gray-500 mb-1">Search</label>
                <input type="text" name="search" value="{{.SearchQuery}}" placeholder="Name, hostname, or group..."
                       class="block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 px-3 py-1.5 border">
            </div>
            <!-- State Filter -->
            <div>
            ...
            {{with .StateFilter}}<input type="hidden" name="state" value="{{.}}">{{end}}
            <button type="submit"
                    class="inline-flex items-center px-4 py-1.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700">
                Search
            </button>
...
<div class="px-4 py-6 sm:px-0">
    ...
    <div class="mb-4">
        ...
        <p class="mt-1 text-sm text-gray-500">{{.Total}} tasks currently queued or running across all submissions</p>
    </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                {{range .Tasks}}
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-xs font-mono text-gray-500 truncate" style="max-width: 180px;" title="{{.ID}}">{{.ID}}</td>
                    ...
                    <td class="px-4 py-3 text-sm text-gray-500">
                        {{with index $.Submissions .SubmissionID}}{{.SubmittedBy}}{{end}}
                    </td>
                    <td class="px-4 py-3 text-xs text-gray-500">{{.ExecutorType}}</td>
                    <td class="px-4 py-3 text-xs text-gray-500">{{formatTime .CreatedAt}}</td>
