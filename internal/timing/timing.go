// Package timing computes the timing projection served by
// GET /api/v1/submissions/{id}/timing (internal/server/handler_timing.go)
// and reused by the web UI's submission timing panel
// (internal/ui/handlers.go). Keeping the math in one place means both
// callers agree on trust rules, aggregate exclusions and the critical-path
// computation — see the per-function docs below for the normative rules.
package timing
...
const (
	...
	KindSkippedIteration = "skipped-iteration"
	KindCancelled        = "cancelled"
)

// MaxDepth bounds ?include_children recursion (defense-in-depth beside the
// visited set — the parent/child graph is a tree by construction).
const MaxDepth = 16
...
// Report is the /timing response body.
type Report struct {
...
// BuildReport assembles the timing view for one submission. The same `now`
// flows through the whole (possibly recursive) report so "so far" durations
// are mutually consistent. logger may be nil (a nil *slog.Logger discards
// silently via slog's default no-op behavior is NOT guaranteed, so callers
// without a logger should pass slog.Default()).
func BuildReport(ctx context.Context, st store.Store, logger *slog.Logger, sub *model.Submission, now time.Time,
	includeChildren bool, visited map[string]bool, depth int) (*Report, error) {
	...
	if sub.WorkflowID != "" {
		...
		if err != nil {
			logger.Warn("timing: workflow unavailable, critical path omitted",
	...
	for _, t := range tasks {
		...
		kids, err := st.GetChildSubmissions(ctx, t.ID)
		if err != nil {
			logger.Warn("timing: child submission lookup failed",
	...
	for _, si := range steps {
		...
		stepRow, dur := BuildStepTiming(si, rows, now)
		report.Steps = append(report.Steps, stepRow)
	...
	if includeChildren && depth < MaxDepth {
		for _, t := range tasks {
			...
			childReport, err := BuildReport(ctx, st, logger, child, now, true, visited, depth+1)
			if err != nil {
				return nil, err
			}
...
func TaskTimingRow(t *model.Task, child *model.Submission, now time.Time) TaskTiming {
	...
	if row.Kind == KindSubworkflow && child != nil && child.State.IsTerminal() && child.CompletedAt != nil {
	...
	if row.Kind == KindTask && row.RunS != nil {
		compute := *row.RunS
		if row.StageInS != nil {
...
func BuildStepTiming(si *model.StepInstance, rows []TaskTiming, now time.Time) (StepTiming, float64) {
	...
	for _, r := range rows {
		...
		if r.Kind == KindSkippedIteration {
