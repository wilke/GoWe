	...
	if noUpload {
		...
		if inputs != nil {
			...
			if graph, err := p.ParseGraphWithBase(packedCWL, cwlDir); err == nil {
				// Resolve using workflow-level input definitions.
				if graph.Workflow != nil {
					inputs = secondaryfiles.ResolveForInputDefs(graph.Workflow.Inputs, inputs, cwlDir)
				}
				// Also resolve using tool-level input definitions. For bare CommandLineTools
				// wrapped in a synthetic workflow, the workflow inputs don't carry secondaryFiles
				// patterns, but the tool's input definitions do.
